...
test("shared navigation persists between Overview and KYC", async ({ page }) => {
  ...
  await expect(page.locator('[data-slot="sidebar"]')).toHaveAttribute("data-state", "collapsed");
  await expect(page.getByRole("heading", { name: "No case data connected" })).toBeVisible();
...
test("detail panel supports keyboard focus, containment, Escape and return", async ({ page }) => {
  ...
  const trigger = page.getByRole("button", { name: "View panel example" });
  await trigger.focus();
  await page.keyboard.press("Enter");
  const dialog = page.getByRole("dialog", { name: "Detail panel example" });
...
test("laptop layout contains table overflow and respects reduced motion", async ({ page }) => {
  ...
  await page.getByRole("link", { name: "Open KYC Case Review foundation" }).click();
  await expect(page.getByRole("button", { name: "View panel example" })).toBeInViewport();
