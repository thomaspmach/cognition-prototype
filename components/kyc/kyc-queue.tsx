...
export function KycQueue({ role }: { role: "viewer" | "reviewer" }) {
  ...
  const columnMap: Record<ColumnId, TableColumn<CaseRecord>> = {
    id: { key: "id", header: "Case", width: "120px", cell: (record) => (
    ...
    customerName: { key: "customerName", header: "Customer", width: "230px", cell: (record) => (
    ...
    country: { key: "country", header: "Country", width: "90px" },
    status: { key: "status", header: "Status", width: "130px", cell: (record) => <CaseStatusBadge status={record.status} /> },
    assigneeName: { key: "assigneeName", header: "Assignee", width: "140px", cell: (record) => record.assigneeName || <span className="text-muted-foreground">Unassigned</span> },
    submittedAt: { key: "submittedAt", header: "Submitted (UTC)", width: "150px", cell: (record) => record.submittedAt.slice(0, 16).replace("T", " ") },
  };
  ...
  return (
    <section className="mt-8" aria-labelledby="queue-title">
      ...
      <div className="my-4 flex items-center justify-between gap-3 text-xs text-muted-foreground">
        <p role="status">{loading ? "Loading cases…" : error ? "Queue unavailable" : `${data.cases.length} matching cases`}</p>
